...
// add base url for web
export const instance = axios.create({ baseURL: process.env.API_URL || "/" });

export const InterceptorsProvider: React.FC<{ children: any }> = ({ children }) => {
  instance.interceptors.request.use((config: AxiosRequestConfig) => {
    // custom typeGuard for strict null check
    if (!isDefinedType(config.headers)) {
      config.headers = {};
    }
